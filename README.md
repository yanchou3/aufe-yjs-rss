...
3. 对**首次出现**的文章会再抓取文章页，把正文（清洗后的 HTML，图片转绝对地址）写入 `<description>`；已处理过的文章记录在 `state/seen.json`，避免每轮重复抓几十个页面触发官网 WAF。首轮预算 40 篇，之后每轮最多补 15 篇。
4. 有变化时自动提交回仓库，阅读器拉取到的始终是最新列表。

> 官网文章页只有日期（如 2026-07-17），全站没有发布时刻，因此 pubDate 统一为当日 0 点（+0800）。
